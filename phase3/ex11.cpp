...
void SearchFF(vector<USER> &user){
    
    for(vector<USER>::size_type i = 0;i < user.size();i++){
        for(vector<string>::size_type j = 0;j < user[i].friend_list.size();j++){
            for(vector<USER>::size_type k = 0;k < user.size();k++){
                if(i != k && user[i].friend_list[j] == user[k].name){
                    for(vector<string>::size_type l = 0;l < user[k].friend_list.size();l++){
                        if(user[k].friend_list[l] == user[i].name){
                            InsertString(user[i].FF_List, user[k].name);
                            InsertString(user[k].FF_List, user[i].name);
                        }
                    }
                }
            }
...
private:
        ...
        for(vector<PAGE>::size_type i = 0;i < page_data.size();i++)
            for(vector<string>::size_type j = 0;j < page_data[i].word.size();j++)
                for(vector<string>::size_type k = 0;k < search_words.size();k++)
                    if(regex_match(page_data[i].word[j],regex(search_words[k])))
                        page_data[i].word_score += 1.0;
    ...
    map<string,double> ReviewScore(string uname){
        ...
        for(vector<string>::size_type i = 0;i < users[key].FF_List.size();i++)
            for(vector<USER>::size_type j = 0;j < users.size();j++)
                if(users[key].FF_List[i] == users[j].name)
...
public:
    ...
    void InsertUser(string new_users){
        ...
        if(!same){
            tmp.name = new_users;
            users.push_back(tmp);
            cout << new_users << " is enrolled as user" << endl;
        }
        else
            cout << new_users << " is already existed as user" << endl;
    }
    ...
    void SetFriend(string name,string fname){
        ...
        if(!in_user || !in_friend){
            cout << "input user is not found" << endl;
            return;
        }
        ...
        cout << fname << " is enrolled as " << name << "'s friend" << endl;
    }
    ...
    void InsertBook(string new_book,bool view = true){
        ...
        if(in)
            cout << new_book << " is enrolled as book" << endl;
        else
            cout << new_book << " is already existed as book" << endl;
    }
    
    void InsertDocument(string new_doc){
        if(InsertString(documents,new_doc)){
            cout << new_doc << " is enrolled as document" << endl;
            ResitPage(new_doc);
            ...
            cout << new_doc << " is already existed as document" << endl;
    }
    ...
    void RemoveUser(string uname){
        ...
        cout << uname << "'s data is removed" << endl;
    }
        ...
        for(vector<USER>::size_type i = 0;i < users.size();i++){
            if(users[i].name == uname){
                if(DeleteString(users[i].friend_list, fname))
                    cout << fname << " is removed from "<< uname << endl;
                else
                    cout << uname << " and " << fname << " is not friend" << endl;
                break;
            }
        }
    }
    ...
    void RemoveBook(string rmbook){
        if(DeleteString(books, rmbook))
            cout << rmbook << "'s page_data is removed" << endl;
        else
            cout << rmbook << " is not found in book list" << endl;
    }
    
    void RemoveDocument(string rmdoc){
        if(DeleteString(documents, rmdoc))
            cout << rmdoc << "'s page_data is removed" << endl;
        else
            cout << rmdoc << " is not found in document list" << endl;
    }
    ...
    void AddReview(string uname ,string book_title, double score){
        ...
        if(!b_exist){
            cout << book_title << " is not found book list" << endl;
            return;
        }
        
        users[key].book_review[book_title] = score;
        cout << "review update is success" << endl;
    }
    ...
    void DeleteReview(string uname, string book_title){
        ...
        for(map<string,double>::iterator it = users[key].book_review.begin();it != users[key].book_review.end();it++){
            if(it->first == book_title){
                ...
                cout << "book review is deleted" << endl;
                return;
            }
        }
        
        cout << uname << " didn't review " << book_title << endl;
    }
    ...
    void InsertWL(string docname){
        ...
        if(docname.find(".html") == string::npos){
            cout << "input word is not document file" << endl;
            return;
        }
        ...
        for(vector<PAGE>::size_type k = 0;k < page_data.size();k++){
            if(page_data[k].name == docname){
                ...
                cout << "page update is finished" << endl;
                return;
            }
        }
    ...
    void DeleteWL(string docname){
        ...
        for(vector<PAGE>::size_type i = 0;i < page_data.size();i++){
            ...
            if(page_data[i].name == docname){
                ...
                if(success)
                    cout << "page update is success" << endl;
                else
                    cout << "input word is not found in page " << docname << endl;
    ...
    void ShowDocumentDetail(string title){
        ...
        cout << title << " is not found document list" << endl;
    }
    ...
    void GetRecBookList(string name, vector<string> &search_words){
        ...
        for(vector<string>::size_type i = 0;i < search_words.size();i++){
            for(vector<string>::size_type j = 0;j < books.size();j++){
                if(regex_match(books[j],regex(search_words[i])))
                    rev[books[j]] += 5.0;
                if(regex_search(books[j],regex(search_words[i])))
                    rev[books[i]] += 3.0;
            }
        }
...
namespace SQLf {
    ...
    void WriteDataBase(sqlite3 *db, DB &DATA){
        ...
        for(vector<USER>::size_type i = 0;i < users.size();i++){
            for(map<string,double>::iterator it = users[i].book_review.begin();it != users[i].book_review.end();it++){
                order = "insert into ReviewList (name, rbook,score) values ('" + users[i].name + "','" + it->first + "'," + to_string(it->second) + ")";
                sqlite3_exec(db,order.c_str(),NULL,NULL,&error);
            }
        }
